    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user:
                ...
                messages.success(request, f"{username}, вход выполнен")
...
def registration(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            user = form.instance
            auth.login(request, user)
            messages.success(request, f"{user.username}, успешно зарегистрирован!")
            return HttpResponseRedirect(reverse("main:index"))
    else:
...
@login_required
def logout(request):
    messages.success(request, f"{request.user.username}, вышли из аккаунта")
    auth.logout(request)
